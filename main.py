...
def whatGoesDown(hexKeyCode):
    x = INPUT(type=INPUT_KEYBOARD,
              ki=KEYBDINPUT(wVk=hexKeyCode))
    user32.SendInput(1, ctypes.byref(x), ctypes.sizeof(x))

def mustComeUp(hexKeyCode):
    x = INPUT(type=INPUT_KEYBOARD,
              ki=KEYBDINPUT(wVk=hexKeyCode,
...
def AltTab():
    ...
    """
    whatGoesDown(VK_MENU)   # Alt
    whatGoesDown(VK_TAB)    # Tab
    mustComeUp(VK_TAB)  # Tab~
    time.sleep(2)
    mustComeUp(VK_MENU) # Alt~

def ti(funnyBunny):
    whatGoesDown(funnyBunny)
    time.sleep(round(random.uniform(0.070, 0.150), 3))
    mustComeUp(funnyBunny)

if __name__ == "__main__":
    while True:
        for i in range (3):
            zzz = round(random.uniform(2.01, 4.99), 2)
            # print(zzz)
            time.sleep(zzz)
